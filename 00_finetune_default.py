...
# Shuffle und Split (32 Train, 4 Val, 4 Test)
random.seed(42)
...
# Splits speichern für spätere Evaluation
Path('checkpoints').mkdir(exist_ok=True)
with open(Path('checkpoints') / 'data_splits.txt', 'w') as f:
    f.write("TRAIN:\n")
    for d in train_dirs:
        f.write(f"  {d.name}\n")
    f.write("\nVAL:\n")
    ...
    def __init__(self, data_dirs):
        ...
        for data_dir in data_dirs:
            try:
                ...
                vel = d['vel']  # (T, C, X, Z) oder (T, X, Z, C)

                # Trajektorie durchlaufen: (t, t+1) Paare
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), Path('checkpoints') / 'pde_transformer_finetuned_best.pt')
        print(f"  -> Neues bestes Modell gespeichert (Val Loss: {val_loss:.6f})")

# Finales Modell speichern
torch.save(model.state_dict(), Path('checkpoints') / 'pde_transformer_finetuned_final.pt')
